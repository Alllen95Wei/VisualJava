...
# 不足之處
在開發的過程中，我們雖然有很多「做出來會很酷」的想法，但礙於時間與技術問題而沒有實作。除此之外，我們在審視自己的成果時，也發現了許多可改進之處。
- 存檔/讀檔功能
    - 我們在介面上設計了一個「Save」按鈕，卻沒有實際的功能。我們苦惱於「應該以什麼格式儲存」，加上其他功能的優先度更高，我們便放棄了這個想法。最後一次進行討論時，我們想到：或許使用 Java 的 `ObjectInput/OutputStream` 直接打包 *工作區* 的 `Pane` 或許可行？
- 使用者自訂函式
    - 在後端，「`Flow`」這個類別之所以會存在，便是為了能一次執行多個 `Step`，因此對後端而言並不是難事；然而，如果要讓使用者能自訂函式，前端便需要面對更多變因，最後也面對和存檔功能一樣的難題——時間不夠而放棄。
- 程式碼風格欠缺統一性
    - 由於我們採取前後分離的開發，因此產出的程式碼自然會帶有不同組員的風格，再加上AI的「協助」後，整個專案的程式碼變得有些凌亂。如果這個專案還要繼續發展，當務之急就是把AI的產物全部重構，讓邏輯更加清晰，最好能全部以OOP重來。
- `interface` or `abstract class`?
  - 後端開始編寫時，採用的是「`interface`」當作所有物件的模板；但是在後期，我認為「`abstract class`」或許能減少幾行重複的定義，也能改進專案的邏輯；可惜，木已成舟。
